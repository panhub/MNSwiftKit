...
public class PickerResourceLoader {
    ...
    private class var bundle: Bundle {
        ...
        if let url = Bundle(for: PickerResourceLoader.self).url(forResource: "MNSwiftKit_AssetPicker", withExtension: "bundle"), let bundle = Bundle(url: url) {
            return bundle
        }
        // 可能是直接源代码集成，尝试在框架 bundle 的上级目录查找
        if let url = Bundle(for: PickerResourceLoader.self).url(forResource: "MNSwiftKit_AssetPicker", withExtension: "bundle", subdirectory: "Frameworks/MNSwiftKit.framework"), let bundle = Bundle(url: url) {
            return bundle
        }
