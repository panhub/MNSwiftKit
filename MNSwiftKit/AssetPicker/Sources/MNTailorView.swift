...
class MNTailorView: UIView {
    ...
    override func layoutSubviews() {
        ...
        pointer.frame = CGRect(x: scrollView.frame.minX, y: 0.0, width: 4.0, height: scrollView.frame.height)
        pointer.frame.origin.y = scrollView.frame.midY - pointer.frame.height/2.0
        pointer.alpha = 0.0
    ...
    func reloadFrames() {
        ...
        let duration: TimeInterval = MNAssetExportSession.seconds(for: videoPath)
        ...
        thumbnailView.frame.size.width = contentSize.width
    ...
    private func adaptLeftMask() {
        ...
        leftMaskView.frame.size.width = max(0.0, max(tailorHandler.leftHandler.frame.maxX - scrollView.frame.minX, 0.0) + contentOffset.x)
    }
    ...
    private func adaptRightMask() {
        ...
        rightMaskView.frame.size.width = max(0.0, max(0.0, scrollView.frame.maxX - tailorHandler.rightHandler.frame.minX) + width)
        rightMaskView.frame.origin.x = contentSize.width - rightMaskView.frame.width
    }
    
    private func adaptPointer(location: CGPoint) {
        pointer.frame.origin.x = max(location.x - pointer.frame.width/2.0, tailorHandler.leftHandler.frame.maxX)
        pointer.frame.origin.x = min(pointer.frame.minX, tailorHandler.rightHandler.frame.minX - pointer.frame.width)
    }
    
    func movePointerToBegin() {
        pointer.frame.origin.x = tailorHandler.leftHandler.frame.maxX
    }
    
    func movePointerToEnd() {
        pointer.frame.origin.x = tailorHandler.rightHandler.frame.minX - pointer.frame.width
    }
}
