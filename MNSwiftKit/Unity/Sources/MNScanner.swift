...
/// 扫码代理
@objc public protocol MNScannerDelegate: NSObjectProtocol {
    /// 开始扫描
    /// - Parameter scanner: 扫码器
    @objc optional func scannerDidStartRunning(_ scanner: MNScanner)
    /// 停止扫描
    /// - Parameter scanner: 扫码器
    @objc optional func scannerDidStopRunning(_ scanner: MNScanner)
    /// 改变手电筒
    /// - Parameter scanner: 扫码器
    @objc optional func scannerDidChangeTorchScene(_ scanner: MNScanner)
    /// 亮度改变
    /// - Parameters:
    ///   - scanner: 扫码器
    ///   - value: 亮度
    @objc optional func scanner(_ scanner: MNScanner, didUpdateBrightness value: Float)
    /// 扫描失败
    /// - Parameters:
    ///   - scanner: 扫码器
    ///   - error: 错误
    @objc optional func scanner(_ scanner: MNScanner, didFail error: Error)
    /// 扫描结果告知
    /// - Parameters:
    ///   - scanner: 扫码器
    ///   - result: 扫描结果
...
public class MNScanner: NSObject {
    /// 事件代理
    public weak var delegate: MNScannerDelegate?
    /// 串行队列
    private let queue: DispatchQueue = DispatchQueue(label: "com.mn.scanner.queue")
    /// 设备
    ...
    /// 请求摄像头权限
    /// - Parameter handler: 请求结果回调
    private func requestCameraPermission(_ handler: @escaping (_ granted: Bool)->Void) {
...
extension MNScanner {
    ...
    public class func readImageMetadata(_ image: UIImage, completion completionHandler: @escaping ((String?)->Void)) {
