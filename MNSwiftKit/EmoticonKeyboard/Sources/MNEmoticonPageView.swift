...
open class MNEmoticonPageView: UIScrollView {
    ...
    public var currentPageIndex: Int {
        get {
            ...
            case .horizontal:
                ...
                guard x.isNormal, x.isNaN == false else { return 0 }
                let w = frame.width
                guard w.isNormal, w.isNaN == false else { return 0 }
                index = Int(round(x/w))
            default:
                let y = contentOffset.y
                guard y.isNormal, y.isNaN == false else { return 0 }
                let h = frame.height
                guard h.isNormal, h.isNaN == false else { return 0 }
                index = Int(round(y/h))
            }
            return index
        }
