...
//  MNSwiftKit
//
//  Created by panhub on 2021/12/10.
//
...
extension MNAssetExporter {
    ...
    /// 获取媒体资源时长
    /// - Parameter path: 媒体资源文件路径
    /// - Returns: 时长
    @objc class func duration(mediaAtPath path: String) -> TimeInterval {
        guard let asset = AVURLAsset(mediaAtPath: path) else { return 0.0 }
    ...
    /// - Parameter path: 媒体资源文件路径
    /// - Returns: 尺寸
    @objc class func naturalSize(videoAtPath path: String) -> CGSize {
    ...
    ///   - maximumSize: 尺寸限制
    /// - Returns: 缩略图
    @objc class func thumbnail(videoAtPath path: String, seconds: TimeInterval = 0.1, maximumSize: CGSize = .zero) -> UIImage? {
        guard let videoAsset = AVURLAsset(mediaAtPath: path) else { return nil }
    ...
    /// - Parameter path: 媒体资源文件路径
    /// - Returns: 封面图片
    @objc class func artwork(mediaAtPath path: String) -> UIImage? {
    ...
    /// - Parameter path: 媒体资源文件路径
    /// - Returns: 简述信息
    @objc class func brief(mediaAtPath path: String) -> [AVMetadataKey:String] {
        guard FileManager.default.fileExists(atPath: path) else { return [:] }
        guard let asset = AVURLAsset(mediaAtPath: path) else { return [:] }
