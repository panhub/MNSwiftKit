...
//  MNFoundation
//
//  Created by MNFoundation on 2022/1/17.
//  字符串加/解密
...
extension MNCrypto {
    ...
    public var md5: String {
        ...
        let calculator = MNMD5(message)
        let md5Data = calculator.calculate()
        let components: [String] = md5Data.compactMap { String(format: "%02x", $0) }
...
private struct MNBytesIterator: IteratorProtocol {
    let data: [UInt8]
    let chunkSize: Int
...
private struct MNBytesSequence: Sequence {
    let data: [UInt8]
    let chunkSize: Int
    
    func makeIterator() -> MNBytesIterator {
        return MNBytesIterator(data: data, chunkSize: chunkSize)
    }
}

private class MNMD5: MNHashConvertible {
    ...
    let shifts: [UInt32] = [7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22,
                                    5, 9, 14, 20, 5, 9, 14, 20, 5, 9, 14, 20, 5, 9, 14, 20,
                                    4, 11, 16, 23, 4, 11, 16, 23, 4, 11, 16, 23, 4, 11, 16, 23,
                                    6, 10, 15, 21, 6, 10, 15, 21, 6, 10, 15, 21, 6, 10, 15, 21]

    // binary integer part of the sines of integers (Radians)
    let sines: [UInt32] = [0xd76aa478, 0xe8c7b756, 0x242070db, 0xc1bdceee,
    ...
    func calculate() -> [UInt8] {
        ...
        let lengthBytes = bytes(with: lengthInBits, totalBytes: 64 / 8)
        tmpMessage += lengthBytes.reversed()
        
        // Process the message in successive 512-bit chunks:
        let chunkSizeBytes = 512 / 8 // 64
        
        for chunk in MNBytesSequence(data: tmpMessage, chunkSize: chunkSizeBytes) {
    ...
    func bytes(with value: Int, totalBytes: Int = MemoryLayout<Int>.size) -> [UInt8] {
        arrayOfBytes(value, length: totalBytes)
    }
