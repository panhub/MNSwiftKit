...
//  MNSwiftKit
//
//  Created by panhub on 2021/7/29.
//  HTTPS信任策略
    ...
    public func evaluate(server trust: SecTrust, domain: String? = nil) -> Bool {
        ...
        case .publicKey:
            if let trustedPublicKeys = MNPublicKeyTrustChainForServerTrust(trust) {
                for trustedPublicKey in trustedPublicKeys {
                    for publicKey in publicKeys {
                        guard trustedPublicKey == publicKey else { continue }
                        return true
                    }
                }
            }
...
private func MNPublicKeyTrustChainForServerTrust(_ serverTrust: SecTrust) -> [SecKey]? {
    ...
    if #available(iOS 15.0, *) {
        guard let certificates = SecTrustCopyCertificateChain(serverTrust) as? [SecCertificate] else { return nil }
        for certificate in certificates {
            var trust: SecTrust!
            guard SecTrustCreateWithCertificates(certificate, policy, &trust) == errSecSuccess else { continue }
            var error: CFError!
            guard SecTrustEvaluateWithError(trust, &error) else { continue }
    ...
    return trustChain.isEmpty ? nil : trustChain
}
