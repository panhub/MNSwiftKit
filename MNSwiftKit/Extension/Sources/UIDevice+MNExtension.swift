...
extension UIDevice {
    ...
    /// 系统版本号
    public static var version: Double { NSDecimalNumber(string: UIDevice.current.systemVersion).doubleValue }
    
    /// 是否为越狱设备
    public static let isBreakDevice: Bool = {
    ...
    @objc public func rotationToInterfaceOrientation(_ orientation: UIInterfaceOrientation, force: Bool = false) {
        guard let deviceOrientation = UIDeviceOrientation(rawValue: orientation.rawValue) else { return }
        rotationToDeviceOrientation(deviceOrientation, force: force)
    }
    
    /// 旋转设备到指定设备方向
    /// - Parameter orientation: 设备方向
    @objc public func rotationToDeviceOrientation(_ orientation: UIDeviceOrientation, force: Bool = false) {
        if force == false, self.orientation == orientation { return }
        setValue(orientation.rawValue, forKey: #keyPath(UIDevice.orientation))
...
public func IOS_VERSION_EQUAL(_ version: Double) -> Bool {
    UIDevice.version == version
}

public func IOS_VERSION_EQUAL(_ version: CGFloat) -> Bool {
    CGFloat(UIDevice.version) == version
}
