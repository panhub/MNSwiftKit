[![Logo](images/logo-degraded.png)](https://github.com/panhub/MNSwiftKit)
...
[![iOS](https://img.shields.io/badge/iOS-12.0+-blue.svg?style=flat)](https://developer.apple.com/ios)
...
[![Maintained](https://img.shields.io/badge/Maintained%3F-yes-green.svg?style=flat)](https://github.com/panhub/MNSwiftKit/graphs/commit-activity)
[![License](https://img.shields.io/cocoapods/l/MNSwiftKit.svg?style=flat)](https://cocoapods.org/pods/MNSwiftKit)

MNSwiftKit 是一系列可复用的 Swift 组件集合，旨在加快 iOS 开发进程。
- 适用于常见场景的自定义用户界面组件;
- 用于原生类型的功能性 Swift 扩展;
- 方便实用的方法和工具;
...
let package = Package(
    ...
    dependencies: [
        .package(url: "https://github.com/panhub/MNSwiftKit.git", from: "0.1.5")
    ],
    targets: [
        .target(
            name: "您的Target名称",
...
```swift
// 使用标题数组创建
let menu = MNPopoverView(titles: "分享", "收藏", "删除", configuration: MNPopoverConfiguration())
menu.popup(in: view, target: button, events: { index in
...
let menu = MNPopoverView(arrangedViews: [button1, button2], configuration: MNPopoverConfiguration())
...
let menu = MNPopoverView(titles: "选项1", "选项2", "选项3", configuration: options)
menu.popup(in: view, target: button, events: { index in
    print("点击了选项")
})
